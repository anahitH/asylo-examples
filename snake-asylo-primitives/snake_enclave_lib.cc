...
#ifdef DEBUG
#define DBG(fmt, args...) insecure_fprintf (stderr, fmt, ##args)
#else
#define DBG(fmt, args...)
#endif
...
extern time_t insecure_time (time_t* timer);
extern void insecure_srand (unsigned int seed);
extern int insecure_rand (void);
extern int insecure_puts (const char* str);
extern int insecure_printf (const char* fmt, ...);
...
void show_score (screen_t *screen)
{
   textcolor (LIGHTCYAN);
   gotoxy (3, MAXROW + 2);
   insecure_printf ("Level: %05d", screen->level);

   textcolor (YELLOW);
   gotoxy (21, MAXROW + 2);
   insecure_printf ("Gold Left: %05d", screen->gold);

   textcolor (LIGHTGREEN);
   gotoxy (43, MAXROW + 2);
   insecure_printf ("Score: %05d", screen->score);

   textcolor (LIGHTMAGENTA);
   gotoxy (61, MAXROW + 2);
   insecure_printf ("High Score: %05d", screen->high_score);
}
...
void setup_level (screen_t *screen, snake_t *snake, int level)
{
   ...
   insecure_srand(insecure_time(NULL));
   ...
   for (i = 0; i < screen->obstacles * 2; i++)
   {
      ...
      do
      {
         row = insecure_rand () % MAXROW;
         col = insecure_rand () % MAXCOL;
      }
   ...
   for (row = 0; row < MAXROW; row++)
   {
      ...
      for (col = 0; col < MAXCOL; col++)
      {
         insecure_printf ("%c", screen->grid[row][col]);
      }
...
#ifdef DEBUG
   gotoxy (71, 1);
   insecure_printf ("(%02d,%02d)", snake->body[snake->len - 1].col, snake->body[snake->len - 1].row);
#endif
}
